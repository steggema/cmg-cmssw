    ...
    def restrictList(self, run, triggerList, isData, isEmbed=False):
        ...
        if run != self.run:
            try:
                #import pdb ; pdb.set_trace()
                if isData :
                  selMenus  = self.menus.findUnprescaledPaths(run, 'MuEG')
 #                 try :
...
#
##        for a in object.getSelectionNames():
##            print 'check : ', a
#
...
#        import pdb; pdb.set_trace()
        ...
        for ii, it in enumerate(object):
            for name in it.getSelectionNames():
                if pattern.match( name ):
                    if it.getSelection( name ) is False:
                        return None, -1

#                    prescale = it.getPrescale( name )
                    prescale = 1
                    if prescale == 1:
                        return name, prescale
                    elif prescale < maxPrescale:
